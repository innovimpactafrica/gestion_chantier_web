...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  dashboardData = {
    ...
    etudes: {
      ...
      livrees: 0,
      total: 0,
      percentageCompletees: 0,
      percentageEnCours: 0,
      percentageInitiees: 0,
      percentageNonInitiees: 0
    },
    avancement: {
      pourcentage: 0,
      display: {
  ...
  private processEtudes(): any {
    ...
    const completees = data.terminees || 0;
    const enCours = data.enCours || 0;
    const initiees = data.livrees || 0;
    const nonInitiees = Math.max(0, total - completees - enCours - initiees);

    return {
      ...data,
      // Dynamic percentages for the distribution chart
      percentageCompletees: Math.round((completees / total) * 100),
      percentageEnCours: Math.round((enCours / total) * 100),
      percentageInitiees: Math.round((initiees / total) * 100),
      percentageNonInitiees: Math.round((nonInitiees / total) * 100)
    };
  }
...
// import { forkJoin, Subject } from 'rxjs';
// import { takeUntil, catchError, filter, switchMap, startWith, delay } from 'rxjs/operators';
...
//   hasError = false;
//   errorMessage = '';
//   isWaitingForAuth = false; // Nouveau flag pour distinguer l'attente d'authentification
...
//   /**
//    * Configure l'écoute de l'état d'authentification
//    */
...
//     //   takeUntil(this.destroy$),
//     //   filter(user => user !== null), // Attendre qu'un utilisateur soit connecté
//     //   switchMap(() => this.loadDashboardDataObservable())
//     // ).subscribe();
...
//     timer(0, 1000).pipe(
//       takeUntil(this.destroy$),
//       filter(() => this.isLoading && !this.hasError),
//       switchMap(() => of(this.authService.currentUser())),
...
//   /**
//    * Vérifie l'authentification et charge les données si possible
//    */
...
//       this.loadDashboardData();
//     } else {
//       // Pas encore connecté, mettre en attente
//       this.isLoading = true;
...
//       timer(30000).pipe(
//         takeUntil(this.destroy$),
//         filter(() => this.isWaitingForAuth)
//       ).subscribe(() => {
//         this.hasError = true;
//         this.errorMessage = 'Timeout: Impossible de charger les données. Veuillez vous reconnecter.';
//         this.isLoading = false;
...
//     if (!this.dashboardService.isUserConnected()) {
//       console.warn('Utilisateur non connecté lors du chargement des données');
//       this.isWaitingForAuth = true;
//       return;
//     }
...
//   /**
//    * Méthode pour réessayer le chargement en cas d'échec
//    */
...
//   // Gestionnaire d'événements clavier (déclaré comme propriété de classe)
...
//       console.log('Données processées avec succès:', this.dashboardData);
//     } catch (error) {
//       console.error('Erreur lors du traitement des données:', error);
//     }
//   }
...
//   // ===== MÉTHODES DE CRÉATION DES GRAPHIQUES (inchangées) =====
...
//   /**
//    * Retourne true si les données sont en cours de chargement
//    */
...
//   /**
//    * Retourne true si le composant attend l'authentification
//    */
//   get isWaitingForAuthentication(): boolean {
...
//    */
//   get dataErrorMessage(): string {
//     return this.errorMessage;
//   }

//   /**
//    * Retourne true si des données ont été chargées avec succès
//    */
//   get hasDataLoaded(): boolean {
//     return !this.isLoading && !this.hasError && !this.isWaitingForAuth;
//   }
...
//     return this.dashboardService.isUserConnected();
//   }
// }
